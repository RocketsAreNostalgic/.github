...
Use the repository's commit and release conventions. Where a project uses Release Please or otherwise requires Conventional Commits, use the appropriate Conventional Commit subject. If a pull request may be squash-merged and its title becomes the final commit consumed by release automation, treat that title as release metadata: a release-significant change must use a release-driving classification recognized by that repository's own release policy. See [RELEASE_CLASSIFICATION.md](RELEASE_CLASSIFICATION.md).
...
Organization-wide requirements and the local-override rules are defined in the [RAN Community Standards](COMMUNITY_STANDARDS.md).
