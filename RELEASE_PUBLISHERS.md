...
- `ran-enhanced-cover`: **outstanding** — bind the write-capable manual recovery path to trusted `main` under `RocketsAreNostalgic/ran-enhanced-cover#20` without weakening its existing exact artifact/deployment contract.

`ran-enhanced-cover#20` is the remaining concrete repository remediation currently identified by this implementation-level publisher audit. Broader release-authority architecture and estate-wide manual-dispatch actor policy remain separately owned by #9/#24.
