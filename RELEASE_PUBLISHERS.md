...
| `ran-enhanced-cover` | WordPress artifact publisher/deployer | Successful same-repository `Quality` `workflow_run`; exact Quality revision and merged Release Please candidate are proven; tested artifacts are carried into immutable publication and optional protected WordPress.org deployment; manual recovery is separately gated to trusted `main` before its write-capable path | Blacksmith Ubuntu 24.04; pinned Actions; project-locked build toolchain | **CONFORMS** | Normal publication remains exact and immutable; repository-local manual-recovery trusted-ref defence completed by `RocketsAreNostalgic/ran-enhanced-cover#20` / PR #21, merge `de493b4a75687eb7fa30f81c98811ce260d2fc1e`. External `workflow_dispatch` actor/credential authority remains organisation-level work under #24. |
| `ran-emailoctopus-jetpack-forms` | WordPress artifact publisher/deployer | Successful same-repository `Quality` `workflow_run`; exact qualified candidate and tested assets are verified before publication/deployment; mutable recovery builds historical source separately and publishes only verified assets | Blacksmith Ubuntu 24.04; pinned Actions; project-locked build toolchain | **CONFORMS** | Mutable-recovery provenance completed by `RocketsAreNostalgic/ran-emailoctopus-jetpack-forms#17` / PR #22. Missing `v2.3.0` publication state is tracked separately by #21. |
| `ran-turnstile-for-jetpack-forms` | WordPress artifact publisher/deployer | Successful same-repository `Quality` `workflow_run`; exact qualified candidate and tested assets are verified before publication/deployment; mutable recovery builds historical source separately and publishes only verified assets | Blacksmith Ubuntu 24.04; pinned Actions; project-locked build toolchain | **CONFORMS** | Mutable-recovery provenance completed by `RocketsAreNostalgic/ran-turnstile-for-jetpack-forms#14` / PR #17. Missing `v0.4.0` publication state is tracked separately by #18. |
...
There are no remaining **REMEDIATION REQUIRED** repositories from the implementation-level publisher audit. Broader release-authority architecture and the estate-wide manual-dispatch actor/credential policy remain separately owned by #9/#24.
