...
| `ran-starter-plugin` | Starter/reference WordPress plugin | Direct `main` push invokes Release Please with workflow-level write authority and no equivalent exact Quality-evidence admission in the release workflow | `ubuntu-latest`; Release Please referenced by mutable `v5.0.0` tag | **REMEDIATION REQUIRED** | Focused migration already tracked by `RocketsAreNostalgic/ran-starter-plugin#17`; do not copy Booster's state machine. |
...
- `ran-starter-plugin`: **open** — migrate its legacy direct-push publisher to the maintained release-trust invariants under `RocketsAreNostalgic/ran-starter-plugin#17`.
