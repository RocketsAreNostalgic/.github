...
- `quality-php-library-v2.yml` runs the repository's Composer quality contract on both its PHP floor and current-stable PHP, with independent PHP syntax verification on the floor lane.
...
- Composer profiles run `composer validate --strict --no-check-publish --no-check-all` before installation where the profile owns the primary quality lane;
- Composer installs use the reviewed lockfile and do not resolve an untracked dependency graph;
- PHP profiles run the repository's `composer check` contract and then, on the primary lane, an independent syntax sweep whose failure propagates out of the workflow;
...
The v2 PHP provider deliberately does not expose a runner input. Pull requests in caller repositories can edit their caller workflow, so runner selection must remain organisation-owned rather than allowing a PR to redirect project-controlled commands to a self-hosted or otherwise privileged runner.
